-- Final Clean Migration: Handles Existing Tables AND Functions
-- Run this ENTIRE file in Supabase SQL Editor
-- ============================================
-- PART 1: Drop Everything (Functions First, Then Tables)
-- ============================================
-- Drop triggers first
DROP TRIGGER IF EXISTS update_student_level ON student_stats;
-- Drop functions
DROP FUNCTION IF EXISTS trigger_update_student_level();
DROP FUNCTION IF EXISTS calculate_student_level(integer);
...
CREATE TABLE questions (
    ...
    question_type TEXT DEFAULT 'mcq' CHECK (
        question_type IN ('mcq', 'true_false', 'fill_blank', 'matching')
    ),
    type_data JSONB,
...
VALUES (
    ...
    (
        'easy_master',
        'Easy Conqueror',
        'Master all easy questions',
        '⭐',
        'mastery',
        75,
        '{"type": "difficulty_master", "level": "easy"}'
    ),
    (
        'medium_master',
        'Challenge Seeker',
        'Master all medium questions',
        '🌟',
        'mastery',
        150,
        '{"type": "difficulty_master", "level": "medium"}'
    ),
    (
        'hard_master',
        'Legend',
        'Master all hard questions',
        '👑',
        'mastery',
        300,
        '{"type": "difficulty_master", "level": "hard"}'
    ),
    (
        'variety_lover',
        'Variety Star',
        'Answer all 4 question types',
        '🎨',
        'milestone',
        40,
        '{"type": "question_types", "value": 4}'
    ),
    (
        'quick_learner',
        'Quick Learner',
    ...
    (
        ...
        'milestone',
        200,
        '{"type": "quiz_count", "value": 20}'
    );
-- ============================================
-- PART 5: Helper Functions
-- ============================================
-- Function to calculate student level from points
CREATE FUNCTION calculate_student_level(points INTEGER) RETURNS INTEGER AS $$ BEGIN IF points <= 100 THEN RETURN 1;
...
-- Trigger function to auto-update level when points change
CREATE FUNCTION trigger_update_student_level() RETURNS TRIGGER AS $$ BEGIN NEW.level = calculate_student_level(NEW.total_points);
NEW.updated_at = timezone('utc'::text, now());
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
-- Create trigger
CREATE TRIGGER update_student_level BEFORE
INSERT
    OR
UPDATE OF total_points ON student_stats FOR EACH ROW EXECUTE FUNCTION trigger_update_student_level();
-- ============================================
